...
AD_NARRATIVE_PROMPT = (
    ...
    "욕구: 이 광고를 본 시청자가 느끼는 소비 욕구를 서술하세요. "
    "(구체적 소비 행동으로 표현. 예: 음식 구매, 금융상품 가입, 여행 예약, 뷰티·패션 구매 등. 감정 단어·상황 묘사 금지)\n\n"
    "예시:\n"
    "상황: 친구들과 함께 소파에 앉아 치킨을 먹으며 즐거운 시간을 보내는 장면이다.\n"
    "감정: 유쾌하고 활기찬 분위기를 전달한다.\n"
    "욕구: 치킨을 즉시 주문하거나 배달앱을 열어보고 싶어진다.\n\n"
...
def _get_unprocessed_ads(limit: int | None, force: bool = False) -> list[dict]:
    if force:
        ...
        sql = """
            SELECT ad_id, ad_name, ad_type, resource_path, ad_category
            FROM ad_inventory
            WHERE target_narrative_v2 IS NULL OR target_narrative_v2 = ''
            ORDER BY ad_id
        """
    params: tuple = ()
    if limit:
...
def _save_narrative(ad_id: str, narrative: str | None) -> None:
    _db.execute(
        "UPDATE ad_inventory SET target_narrative_v2=%s WHERE ad_id=%s",
        (narrative, ad_id),
    )
