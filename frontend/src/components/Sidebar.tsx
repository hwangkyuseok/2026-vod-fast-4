...
interface SidebarProps {
  focusedIndex?: number | null; // null = 사이드바에 포커스 없음
}

export default function Sidebar({ focusedIndex = null }: SidebarProps) {
  const router = useRouter();
  const [hoveredItem, setHoveredItem] = useState<string | null>(null);
  const [activeSection, setActiveSection] = useState<string>("fastvod");
  ...
  return (
    ...
    >
      {NAV_ITEMS.map((item, idx) => {
        ...
        const isRemoteFocused = focusedIndex === idx;
        ...
        return (
          <button
            ...
            style={{
              color: isRemoteFocused || isActive || isHovered ? "#FFFFFF" : "#8892A4",
              background: isRemoteFocused
                ...
                : "transparent",
              borderLeft: isRemoteFocused
                ? "3px solid #FFFFFF"
                : isActive
                ? "3px solid #E60012"
                : "3px solid transparent",
              transform: isRemoteFocused ? "scale(1.1)" : isHovered ? "scale(1.13)" : "scale(1)",
